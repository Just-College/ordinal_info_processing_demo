...
RESULT_DIR = f"{Path(__file__).parent.parent}/figure/"
...
def forward_with_states(model, input_seq):
    ...
    return torch.cat(outputs, dim=1), torch.cat(rates, dim=1), torch.cat(states, dim=1)


def collect_states_by_class(model):
...
def main():
    ...
    for node_name, node_state, nearest_node_distance in zip(
        ...
        near_ratio = result["state_distance"] / max(nearest_node_distance, 1e-12)
        is_fixed = (
        ...
        rows.append(
            {
                "node": node_name,
                "initial_residual_norm": result["initial_residual"],
                "final_residual_norm": result["final_residual"],
                "final_residual_rms": result["final_residual_rms"],
                "state_distance_to_node": result["state_distance"],
                "rate_distance_to_node": result["rate_distance"],
                "nearest_node_distance": nearest_node_distance,
                "distance_over_nearest_node_distance": near_ratio,
                "fixed_point_found": is_fixed,
                "step_spectral_radius": stability["step_spectral_radius"],
                "continuous_max_real_eig": stability["continuous_max_real_eig"],
                "discrete_stable": stability["discrete_stable"],
                "continuous_stable": stability["continuous_stable"],
                "output": result["output"].tolist(),
            }
        )

    out_path = f"{RESULT_DIR}/sec2_fixed_point_test.csv"
    ...
    print(f"Saved fixed-point test: {out_path}")
    print(
        "node initial_residual final_rms state_dist near_ratio spectral_radius max_real discrete_stable continuous_stable fixed_point"
    )
    for row in rows:
        print(
            f"{row['node']:>4} "
            f"{row['initial_residual_norm']:.6g} "
            f"{row['final_residual_rms']:.6g} "
            f"{row['state_distance_to_node']:.6g} "
            f"{row['distance_over_nearest_node_distance']:.3f} "
            f"{row['step_spectral_radius']:.6g} "
            f"{row['continuous_max_real_eig']:.6g} "
            f"{row['discrete_stable']} "
            f"{row['continuous_stable']} "
            f"{row['fixed_point_found']}"
        )

    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for x, y in loader_test:
                ...
                # 第二个音素区间
                pred2 = (
                    out[:, 30 + 3 + 30 : 30 + 3 + 30 + 3, :].mean(dim=1).argmax(dim=1)
                )
                target2 = (
                    y[:, 30 + 3 + 30 : 30 + 3 + 30 + 3, :].mean(dim=1).argmax(dim=1)
                )
        ...
        print(
            f"Epoch {epoch+1} loss: {train_loss:.4f}, test_loss: {test_loss_avg:.4f}, test_acc: {test_acc:.2f}%, lr: {scheduler.get_last_lr()[0]:.6f}"
        )
        history.append(
            [epoch + 1, train_loss, test_loss_avg, test_acc, scheduler.get_last_lr()[0]]
        )
    csv_path = os.path.join(
        csv_dir, f"train_history_{params_str}_{time.strftime('%Y%m%d_%H%M%S')}.csv"
    )
